...
# Proof-of-concept: convert a raw SQL schema (PRAGMA + 2 tables + 1 index)
...
#
# Lives under examples/ so it can be re-run any time the schema evolves.
...
pragma_value = db.query('PRAGMA foreign_keys').first.values.first
...
SQL
last_child_id = db.last_insert_rowid

db.execute(<<~SQL, last_child_id)
  INSERT INTO journal_entries (media_node_id, content, created_at, updated_at)
  VALUES (?, 'Hello world', '2024-01-01', '2024-01-01');
SQL
last_journal_id = db.last_insert_rowid

puts ''
puts "Created: parent=#{last_id}, child=#{last_child_id}, journal=#{last_journal_id}"

db.execute('DELETE FROM media_nodes WHERE id = ?', last_id)
puts "After deleting parent #{last_id}:"

media_remaining = db.query('SELECT id FROM media_nodes ORDER BY id').map { |r| r[:id] }
journal_remaining = db.query('SELECT id FROM journal_entries ORDER BY id').map { |r| r[:id] }
...
SQL
fresh_parent_id = db.last_insert_rowid
puts "Fresh parent id=#{fresh_parent_id}"

db.execute(<<~SQL, fresh_parent_id)
  INSERT INTO media_nodes (parent_id, title, kind, position, status, created_at, updated_at)
